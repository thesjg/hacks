...
class server_config(sql.SQLNotifyTable):
    ...
    def INSERT(self, row_id, new_dict):
        ...
        self.ServerDB.Query(new_dict['server_id'], query)
...
class users(sql.SQLNotifyTable):
    ...
    def INSERT(self, row_id, new_dict):
       ...
       server_id = self.DB.Query(self.ServerIDQuery)[0][0]

         # Get max gid for server
       max_gid = self.DB.Query("""SELECT max_gid FROM server_config WHERE
                                   server_id='%d'""" % (server_id))[0][0]

         # Get the current max gid on that server
       current_max_gid = self.DB.Query("""SELECT max(server_gid) FROM mail_gid_trans
       ...
       if current_max_gid == '':
           new_gid = self.DB.Query("""SELECT min_gid FROM server_config WHERE
                                      server_id='%d'""" % (server_id))[0][0]
       ...
       quota = self.DB.Query(query)[0][0]

        # Snag DBA
       dba = self.DB.Query("SELECT name FROM dba WHERE id='%d'" % new_dict['dba_id'])[0][0]
       ...
       query = """INSERT INTO mail_users (gid, dba, username, quota_bytes) VALUES
                  ('%s', '%s', '%s', '%s')""" % (new_gid, dba, new_dict['username'],
                                                 quota)
       self.ServerDB.Query(server_id, query)
    ...
    def UPDATE(self, row_id, old_dict, new_dict):
        ...
        server_id = self.DB.Query(self.ServerIDQuery % (row_id))[0][0]
        self.ServerDB.Query(server_id, query)
...
class dns_zones(sql.SQLNotifyTable):
    ...
    def UPDATE(self, row_id, old_dict, new_dict):
        ...
        server_id = self.DB.Query(query)[0][0]
        ...
        for i in uids:
            ...
            print query
            result = self.ServerDB.Query(server_id, query)
...
class dns_resources(sql.SQLNotifyTable):
    ...
    def UPDATE(self, row_id, old_dict, new_dict):
        ...
        origin = self.DB.Query(query)[0][0]
...
class mail_users(sql.SQLNotifyTable):
    ...
    def INSERT(self, row_id, new_dict):
       ...
       server_id = self.DB.Query(self.ServerIDQuery % (new_dict['user_id']))[0][0]

         # Get max gid for server
       max_uid = self.DB.Query("""SELECT max_uid FROM server_config WHERE
                                   server_id='%d'""" % (server_id))[0][0]

         # Get the current max gid on that server
       current_max_uid = self.DB.Query("""SELECT max(server_uid) FROM mail_uid_trans
                                          WHERE server_id='%d'""" % (server_id))[0][0]
       ...
       if current_max_uid == None:
           new_uid = self.DB.Query("""SELECT min_uid FROM server_config WHERE
                                      server_id='%d'""" % (server_id))[0][0]
    ...
    def DELETE(self, row_id, old_dict):
         ...
         # Get target server id
       server_id = self.DB.Query(self.ServerIDQuery % (old_dict['user_id']))[0][0]
